...
void construct_path_3(int Paths[7][N]){
  Paths[1][3] = N - 2;
  three_skip_connection(Paths,1,N - 2,3,1,1);
  three_skip_connection(Paths,3,d2,2,1,1);
  three_skip_connection(Paths,4,N - d2,2,1,1);
  three_skip_connection(Paths,5,N - 3,2,1,1);
  three_skip_connection(Paths,6,N - 1,2,1,1);
}
...
void construct_path_d2(int Paths[7][N]){
  ...
  if(d2 % 3 == 1){
    ...
    if(N - d2 == V + 1 && N - 1 == V + d2){
      ...
      three_skip_connection(Paths,4,N - d2,2,1,2);
      three_skip_connection(Paths,5,N - 3,2,1,2);
      Paths[6][3] = N - 2;
      three_skip_connection(Paths,6,N - 2,3,1,2);
    }
  }else if(d2 % 3 == 2){
    Paths[1][3] = 2;
    designation_three_skip_connection(Paths,1,2,3,2,V);
    ...
    Paths[2][i] = V - 1;
    Paths[2][i + 1] = V;
    if(V + d2 == N - 2) three_skip_connection(Paths,4,N - d2,2,2,1);
    else three_skip_connection(Paths,4,N - d2,2,1,1);
    three_skip_connection(Paths,5,N - 3,2,1,1);
    three_skip_connection(Paths,6,N - 1,2,1,1);
  }else if(d2 % 3 == 0){
    Paths[1][3] = 2;
    ...
    if(N - d2 == V + 3 && N - 3 == V + d2){
      ...
      three_skip_connection(Paths,4,N - d2,2,1,3);
      Paths[5][3] = N - 2;
      three_skip_connection(Paths,5,N - 2,3,1,3);
      three_skip_connection(Paths,6,N - 1,2,1,3);
    }
  }
}
...
void construct_path_largethan_d2(int Paths[7][N]){
  ...
  if(d2 % 3 == 1){
    if(V - d2 == 1 && V - 1 == d2){
      ...
      Paths[1][3] = 2;
      three_skip_connection(Paths,1,2,3,2,2);
      three_skip_connection(Paths,2,3,2,2,2);
      three_skip_connection(Paths,3,d2,2,2,2);
    }
    if(N - d2 == V + 1 && N - 1 == V + d2){
      Paths[4][3] = V;
      Paths[5][3] = N - 2;
    ...
    three_skip_connection(Paths,1,1,2,2,1);
    three_skip_connection(Paths,2,3,2,2,1);
    if(V - d2 == 2) three_skip_connection(Paths,3,d2,2,1,1);
    else three_skip_connection(Paths,3,d2,2,2,1);
    if(V + d2 == N - 2) three_skip_connection(Paths,4,N - d2,2,2,1);
    else three_skip_connection(Paths,4,N - d2,2,1,1);
    three_skip_connection(Paths,5,N - 3,2,1,1);
    three_skip_connection(Paths,6,N - 1,2,1,1);
    ...
    if(V - d2 == 3 && V - 3 == d2){
      ...
      three_skip_connection(Paths,1,1,2,2,3);
      Paths[2][3] = 2;
      three_skip_connection(Paths,2,2,3,2,3);
      three_skip_connection(Paths,3,d2,2,2,3);
    }
    if(N - d2 == V + 3 && N - 3 == V + d2){
      Paths[4][3] = V;
      Paths[5][3] = V;
