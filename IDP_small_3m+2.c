...
int main(void){
  ...
  for(i = 1;6 >= i;i++){
    printf("道%d : < ",i);
    for(j = 1;N > j;j++) if(P[i][j] != 0 || j == 1) printf("%d ",P[i][j]);
    printf(">\n");
  }
}
...
void construct_path_lessthan_3(int Paths[7][N]){
  ...
  Paths[3][2] = d2;optimum_connection(Paths,3,d2,2,V + 3);
  Paths[4][2] = N - d2;optimum_connection(Paths,4,N - d2,2,V + d2);
  Paths[5][2] = N - 3;optimum_connection(Paths,5,N - 3,2,N - d2 + V);
}

void construct_path_3(int Paths[7][N]){
  Paths[1][2] = 1;Paths[1][3] = N - 2;Paths[2][2] = 3;Paths[3][2] = d2;
  Paths[4][2] = N - d2;Paths[5][2] = N - 3;Paths[6][2] = N - 1;
...
void construct_path_lessthan_d2(int Paths[7][N]){
  int i = 0;int used_check[N] = {0};used_check[0] = 1;
  if(V % 3 == 1){
    Paths[1][2] = 1;designation_three_skip_connection(Paths,1,1,2,2,V);
    Paths[2][2] = 3;i = designation_three_skip_connection(Paths,2,3,2,2,V - 1);Paths[2][i + 1] = V;
    Paths[3][2] = d2;i = designation_three_skip_connection(Paths,3,d2,2,1,V + 1);Paths[3][i + 1] = V;
    Paths[6][2] = N - 1;i = designation_three_skip_connection(Paths,6,N - 1,2,1,N - d2 + V);Paths[6][i + 1] = V;
    Paths[4][2] = N - d2;Paths[5][2] = N - 3;
    ...
    if(N % 3 == 0){
      ...
      i = one_and_three_skip_connection(Paths,used_check,5,N - 3,2,1,1,V + d2);Paths[5][i + 1] = V;
    }else if(N % 3 == 2){
      i = designation_three_skip_connection(Paths,4,N - d2,2,1,V + d2);Paths[4][i + 1] = V;
      one_and_three_skip_connection(Paths,used_check,5,N - 3,2,1,1,V);
    }
  }else if(V % 3 == 2){
    Paths[1][2] = 1;Paths[1][3] = 2;designation_three_skip_connection(Paths,1,2,3,2,V);
    Paths[2][2] = 3;i = designation_three_skip_connection(Paths,2,3,3,2,V - 2);Paths[2][i + 1] = V - 1;Paths[2][i + 2] = V;
    Paths[3][2] = d2;designation_three_skip_connection(Paths,3,d2,2,1,V);
    Paths[5][2] = N - 3;i = designation_three_skip_connection(Paths,5,N - 3,2,1,N - d2 + V);Paths[5][i + 1] = V;
    Paths[4][2] = N - d2;Paths[6][2] = N - 1;
    trans_Array_used_check(Paths,used_check);
    if(N % 3 == 0){
      i = designation_three_skip_connection(Paths,4,N - d2,2,1,V + d2);Paths[4][i + 1] = V;
      i = one_and_three_skip_connection(Paths,used_check,6,N - 1,2,1,-1,V + 1);Paths[6][i + 1] = V;
    }else if(N % 3 == 1){
      i = one_and_three_skip_connection(Paths,used_check,4,N - d2,2,1,2,V + 1);Paths[4][i + 1] = V;
      i = one_and_three_skip_connection(Paths,used_check,6,N - 1,2,1,2,V + d2);Paths[6][i + 1] = V;
    }else if(N % 3 == 2){
      i = designation_three_skip_connection(Paths,4,N - d2,2,1,V + 1);Paths[4][i + 1] = V;
      i = designation_three_skip_connection(Paths,6,N - 1,2,1,V + d2);Paths[6][i + 1] = V;
    }
  }else if(V % 3 == 0){
    Paths[1][2] = 1;Paths[1][3] = 2;i = designation_three_skip_connection(Paths,1,2,3,2,V - 1);Paths[1][i + 1] = V;
    Paths[2][2] = 3;designation_three_skip_connection(Paths,2,3,3,2,V);
    Paths[3][2] = d2;i = designation_three_skip_connection(Paths,3,d2,2,1,V + 2);Paths[3][i + 1] = V + 1;Paths[3][i + 2] = V;
    Paths[6][2] = N - 1;Paths[6][3] = N - 2;i = designation_three_skip_connection(Paths,6,N - 2,3,1,N - d2 + V);Paths[6][i + 1] = V;
    Paths[4][2] = N - d2;Paths[5][2] = N - 3;
    trans_Array_used_check(Paths,used_check);
    if(N % 3 == 0){
      one_and_three_skip_connection(Paths,used_check,4,N - d2,2,1,1,V);
      ...
      i = designation_three_skip_connection(Paths,4,N - d2,2,1,V + d2);Paths[4][i + 1] = V;
      i = one_and_three_skip_connection(Paths,used_check,5,N - 3,2,1,1,V);
    }else if(N % 3 == 2){
      designation_three_skip_connection(Paths,4,N - d2,2,1,V);
      i = designation_three_skip_connection(Paths,5,N - 3,2,1,V + d2);Paths[5][i + 1] = V;
    }
  }
}
...
void construct_path_largethan_d2(int Paths[7][N]){
  Paths[1][2] = 1;Paths[2][2] = 3;Paths[3][2] = d2;
...
int one_and_three_skip_connection(int Paths[7][N],int used_check[N],int num,int current_location,int i,int type,int check_num,int dest){
  int check = check_num;if(check_num == -1) check = 1;int count = 0;
  while(1){
